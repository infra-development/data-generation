...
class ZookeeperConfigProvider extends ConfigProvider {
  ...
  override def updateBusinessConfig[T](
                                        path: String,
                                        updatedConfig: T,
    ...
    for {
      ...
      // Just to validate: parse existing config (optional, can be removed if unnecessary)
      _ <- parser.parse(content).left.map { err =>
        logger.error(s"Failed to parse existing config at path: $path", err)
        err
      }

      // Serialize the new config (already updated outside)
      serialized <- parser.serialize(updatedConfig).left.map { err =>
        logger.error(s"Failed to serialize updated config at path: $path", err)
        err
      }

      // Write the updated content back to ZooKeeper
      _ <- Try {
